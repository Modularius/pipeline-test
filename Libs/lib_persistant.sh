...
trace_to_events_params() {
    echo "--broker $g_BROKER --consumer-group $g_GROUP_EVENT_FORMATION \
...
aggregator_params() {
    echo "--broker $g_BROKER --group $g_GROUP_AGGREGATOR \
...
nexus_writer_params() {
    echo "--broker $g_BROKER --consumer-group $g_GROUP_WRITER \
