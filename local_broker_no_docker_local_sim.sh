...
INPUT=simulator
SIMULATOR_CONFIG_SOURCE="simulator_configs/run.json"
...
nix-shell -p docker-compose --run 'sudo systemctl start docker && docker-compose --env-file ./configs/.env.local -f "./configs/docker-compose.yaml" --profile=broker up -d'
